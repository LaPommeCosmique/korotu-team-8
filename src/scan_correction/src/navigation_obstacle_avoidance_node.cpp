/**
 * A program that takes in data for the HectorSLAM map, position and orientation of drone, and the target destination determined by the sensor team. This program uses those data with relevant functions to determine the target velocity and target coordinate of the drone. This program publishes the determined data to topics for target velocity and target coordinate.
 
 * @author Mihir Ravishankar, UBC MECH 457 Capstone Team 8
...
// [To be edited for consistency with the specified input data to the program]
...
// [To be edited for specific node objectives]
...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "navigation_obstacle_avoidance_node");
...
// Current z-axis angular velocity (yaw rate) according to IMU data
static long yaw_rate = 0;

// Time stamp of current yaw and yaw_rate
static int time_sec = 0; // seconds since epoch
static int time_nsec = 0; // nanoseconds since time_sec
...
void ObstacleDetectionCallBack(const std_msgs::String::ConstPtr& msg, std::string topicName)
{

// Code for all data types being subscribed to.
...
void imuMessageCallback(const sensor_msgs::Imu::ConstPtr& imuMsg) {
	...
	// get z-axis angular velocity (yaw rate)
	yaw_rate = angular_velocity.z;

	// get time stamp
	time_sec = header.stamp.sec;
	time_nsec = header.stamp.nsec;
}
...
void scanMessageCallback(const sensor_msgs::LaserScan::ConstPtr& scanMsg) {
	...
	// Calculate current yaw based on previous IMU data
	double delta_sec = 1.0 * (header.stamp.sec - time_sec) + 1.0 * (header.stamp.nsec - time_nsec) * 10**-9;
	double current_yaw = yaw + delta_sec * yaw_rate;

	// Offset angle_min and angle_max with current yaw
	angle_min += current_yaw;
	angle_max += current_yaw;
